...
if __name__ == "__main__":
    ...
    ''' # Instead of solving in non linearly (even though it will converge on the first iteration as the problem is linear), we'll do it in a linear solv
    U_initial = np.zeros(V.ndofs)
    U_final = solve_newton_raphson(U_initial, assembler.assemble, apply_bcs)
    '''
    ...
    export_vtu(V, U_final, "results/poisson_linear.vtu", field_name="Scalar Field")
    export_vtu(V, U_final, "results/poisson_linear_error.vtu", field_name="Error", n_vis_pts=4, exact_func=u_exact)